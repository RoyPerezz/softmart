...
{
    public partial class CalificarCubreAlmacen : Form
    {
        ...
        const int DEFICIENTE = 0; const int MEDIO_SUPERIOR = 60; const int SUPERIOR = 80; const int MUY_SUPERIOR = 100;
        const int INFERIOR = 20; const int MEDIO_INFERIOR = 40;


        public CalificarCubreAlmacen()
        {
            InitializeComponent();
        }
        ...
        private void BT_calificar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("?cubre",CB_cubreJefeAlmacen.ToString());
...
#pragma warning restore CS0168 // La variable 'ex' se ha declarado pero nunca se usa
        ...
        private void CalificarCubreAlmacen_Load(object sender, EventArgs e)
        {
            try
            {
                MySqlConnection conexion = BDConexicon.BodegaOpen();
                string query = "SELECT * FROM rd_registro_jefes WHERE puesto='CUBRE JEFE ALMACEN'";
                ...
                while(dr.Read())
                {
                    CB_cubreJefeAlmacen.Items.Add(dr["nombre"].ToString());
                }
                dr.Close();
                conexion.Close();
            }
